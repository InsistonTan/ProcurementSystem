...
public interface GoodsDao {
    ...
    //增加货品
    @Insert("INSERT INTO goods(goods_id,goods_sort,goods_type_id,goods_name,order_unit,rec_unit,sale_unit,can_order,`desc`) " +
            "VALUES(#{goods_id},#{goods_sort},#{goods_type_id},#{goods_name},#{order_unit},#{rec_unit},#{sale_unit},#{can_order},#{desc})")
    boolean add(Goods goods);

    //(根据id)更新货品
    @Update("UPDATE goods SET goods_sort=#{goods_sort},goods_type_id=#{goods_type_id},goods_name=#{goods_name}," +
            "order_unit=#{order_unit},rec_unit=#{rec_unit},sale_unit=#{sale_unit},can_order=#{can_order},`desc`=#{desc} " +
