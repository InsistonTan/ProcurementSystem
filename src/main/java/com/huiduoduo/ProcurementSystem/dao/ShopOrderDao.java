...
public interface ShopOrderDao {
    ...
    List<ShopOrder> selectHistoryByShopID(@Param("id") String shop_id);
    ...
    List<ShopOrder> selectOnGoingByShopID(@Param("id") String shop_id);
    ...
    //（采购经理）修改订单信息
    @Update("update shop_order set approved=#{approved},manager_note=#{manager_note},manager=#{manager}" +
            "where order_id=#{order_id}")
